...
//   // }, []);
//   const calculateActiveIndex = useCallback((progress: number, total: number) => {
//   if (total === 0) return 0;

//   const rawIndex = progress * (total - 1);
//   return Math.min(Math.floor(rawIndex), total - 1);
// }, []);
...
//     const totalCards = projects[0]?.projectItems?.length || 0;

//     setScrollProgress(latest);

//     const newIndex = calculateActiveIndex(latest, totalCards);
...
// }, [smoothProgress, calculateActiveIndex, activeIndex, projects]);
...
//   return (
//     <section className="project-section py-5 mb-3 mt-3" ref={containerRef}>
//        {projects.map((proj,k) =>
...
//                     onClick={() => {
//                       const progress = idx / (proj.projectItems.length - 1);
...
//                    {activeIndex + 1} / {proj.projectItems.length}
//               </span>
//             </div>
//           </div>
//         </div>
...
//                       setActiveIndex(newIndex);
//                       setScrollProgress(newIndex / (proj.projectItems.length - 1));
//                     }}
//                   />
//                   <motion.div 
...
//                     transition={{ repeat: Infinity, duration: 2 }}
//                     onClick={() => {
//                       const newIndex = Math.min(activeIndex + 1, proj.projectItems.length - 1);
//                       setActiveIndex(newIndex);
//                       setScrollProgress(newIndex / (proj.projectItems.length - 1));
//                     }}
//                   />
...
const ProjectCardSection: React.FC = () => {
  ...
  // Wheel scroll for cards
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      const delta = e.deltaY;
      setScrollProgress(prev => Math.min(Math.max(prev + delta * 0.0005, 0), 1));
    };
  ...
  const reconstructFromChunks = (chunkedString: string | null | undefined): string | null => {
    if (!chunkedString) return null;
    return chunkedString.includes(DELIMITER) ? chunkedString.split(DELIMITER).join('') : chunkedString;
  };
        ...
        const processedProjects: Projects[] = projectsData.map((project: DatabaseProject) => {
          const projectItems: ProjectItem[] = (allItemsData || [])
            ...
            .map((item: DatabaseProjectItem) => ({
              ...
              heading: item.heading,
              btn_text: item.btn_text,
              imageUrl: reconstructFromChunks(item.image),
              project_id: item.project_id
            }));
  ...
  return (
    <section className="project-section py-5 mb-5 mt-5" ref={containerRef} style={{ minHeight: '100vh' }}>
      {projects.map((proj, k) =>
        <div key={k} className="container py-5">
          <div className="row mb-5">
            <div className="col-12 text-center">
          ...
          <div className="row justify-content-center">
            <div className="col-lg-10 col-xl-9">
              <div className="project-cards-container" ref={cardsContainerRef} style={{ height: '500px', position: 'relative' }}>
                {proj.projectItems.map((item, index) => (
                  <ProjectCard
                    key={item.id}
                    id={parseInt(item.id)}
